...
const Chat = () => {
  ...
  const check = "you are logged in";
  useEffect(() => {
    // Whenver page loads mount the socket
    console.log(createConnection);
    const socket = createConnection();
    socket.emit("joinChat", { check });
  ...
  return (
    <div className="p-10 min-h-screen flex items-center justify-start flex-col">
      ...
      <div className="w-1/3 border border-zinc-300 rounded-md h-[75vh] overflow-scroll relative">
        {messages &&
        ...
        <div className="send-div flex justify-between p-10 absolute bottom-0 w-full gap-4">
          ...
          />
          <button className="px-4 py-1.5 bg-emerald-500 text-white rounded-md">
            send
          </button>
        </div>
      </div>
